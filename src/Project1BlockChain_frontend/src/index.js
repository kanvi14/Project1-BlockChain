...
import { Project1BlockChain_backend } from "../../declarations/Project1BlockChain_backend";
...
appointForm.addEventListener("submit", async (e) => {
  ...
  try {
    const getId = await Project1BlockChain_backend.addTodo(obj.textarea);
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    const getAll = await Project1BlockChain_backend.getAllTodo();
...
async function deleteTodo(userId) {
  try {
    ...
    await Project1BlockChain_backend.deleteTodo(userId);
